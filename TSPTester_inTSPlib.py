...
class TSPTester():
    ...
    def run(self):
        ...
        for i in range(len(tsplib_names)):
            ...
            if TSPLIB_or_Nation:
                ...
                score_teacher, score_student,problems_size, tour = self._test_one_batch(episode, 1, clock=self.time_estimator_2, Name=name, TSPLIB_or_Nation=TSPLIB_or_Nation, opt_len=torch.tensor(float(opt_len)))
            else:
                # for nationalTSP
                opt_len = national_collections[name]
                score_teacher, score_student, problems_size, tour = self._test_one_batch(episode, 1,
    ...
    def _test_one_batch(self, episode, batch_size, clock=None, Name=None, opt_len=None, TSPLIB_or_Nation=False):

        self.model.eval()

        with torch.no_grad():

            self.env.load_problems(episode, batch_size, Name=Name, opt_len=opt_len, TSPLIB_or_Nation=TSPLIB_or_Nation)
            ...
            self.logger.info("greedy, name:{}, gap:{:5f} %, stu_l:{:5f} , opt_l:{:5f}".format(
                name, gap, current_best_length.mean().item(), self.optimal_length.mean().item()))
            ...
            if self.tester_params['beam']:
                ...
                zero_to_bsz = torch.arange(batch_size, dtype=torch.long, device=current_best_length_beam.device)
                ...
                self.logger.info("beam, name:{}, gap:{:5f} %,  stu_l:{:5f} , opt_l:{:5f}".format(
                    name, gap, current_best_length.mean().item(), self.optimal_length.mean().item()))
            torch.cuda.empty_cache()
            ...
            return self.optimal_length.mean().item(), current_best_length.mean().item(), self.env.problem_size, best_select_node_list
