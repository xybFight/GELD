...
class Beamsearch(object):
    def __init__(self, beam_size, batch_size, num_nodes,
                 ...
                 probs_type='raw', random_start=False, device=torch.device('cuda')):
        """
        Args:
            beam_size: Beam size
        ...
        self.start_nodes = torch.zeros(batch_size, beam_size, device=device).type(self.dtypeLong)
        ...
        self.mask = torch.ones(batch_size, beam_size, num_nodes, device=device).type(self.dtypeFloat)
        self.update_mask(self.start_nodes)  # Mask the starting node of the beam search
        # Score for each translation on the beam
        self.scores = torch.zeros(batch_size, beam_size,  device=device).type(self.dtypeFloat)
        self.all_scores = []
    ...
    def advance(self, trans_probs, selected_nodes):
        ...
        if len(self.prev_Ks) > 0:
            ...
            if self.probs_type == 'raw':
                beam_lk[:, 1:] = torch.zeros(beam_lk[:, 1:].size(), device=self.device).type(self.dtypeFloat)
            elif self.probs_type == 'logits':
                beam_lk[:, 1:] = -1e20 * torch.ones(beam_lk[:, 1:].size(), device=self.device).type(self.dtypeFloat)
    ...
    def get_hypothesis(self, k):
        ...
        hyp = -1 * torch.ones(self.batch_size, self.num_nodes, device=self.device).type(self.dtypeLong)
